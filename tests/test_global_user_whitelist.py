...
async def test_global_whitelist_is_ignored_when_feature_is_disabled(
    db_init: Any,
    ...
    assert await is_user_globally_whitelisted(user_tid, redis=test_services.redis) is False


async def test_global_whitelist_add_check_remove_is_idempotent(
    db_init: Any,
    test_services: ApplicationServices,
) -> None:
    del db_init
    user_tid = 700_000_001
    await set_enabled("global_user_whitelist", True, redis=test_services.redis)
    ...
    try:
        ...
        assert await is_user_globally_whitelisted(user_tid, redis=test_services.redis) is True
        ...
        await delete_override("global_user_whitelist", redis=test_services.redis)
