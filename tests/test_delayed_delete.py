...
async def test_delete_messages_after_delay_uses_bot_delete_messages(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    delete_mock = AsyncMock()
    sleep_mock = AsyncMock()
    service = DelayedDeletionService(
        SimpleNamespace(delete_messages=delete_mock),
    )

    monkeypatch.setattr("sophie_bot.modules.utils_.delayed_delete.asyncio.sleep", sleep_mock)

    await service.delete_messages_after_delay(
        CHAT_TID,
        [1, 2, 3],
        delay_seconds=7,
    )
...
async def test_schedule_message_deletion_retains_and_clears_task(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    delete_mock = AsyncMock()
    ...
    service.schedule(CHAT_TID, [1], delay_seconds=99)
...
async def test_schedule_message_deletion_ignores_empty_list() -> None:
    service = DelayedDeletionService(
        SimpleNamespace(delete_messages=AsyncMock()),
    )

    service.schedule(CHAT_TID, [])
