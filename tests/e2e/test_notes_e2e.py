...
async def test_search_notes_success(
    ...
    """Searching notes finds matching results."""
    monkeypatch.setattr("sophie_bot.modules.notes.utils.send.bot", test_client.bot)
    ...
    user_wrapper, group_chat, chat_model = await _setup_group_and_user(
        ...
        user_id=928000011,
        group_title="Notes Search Group",
        first_name="NoteSearcher",
        username="note_searcher",
    )

    note_match = await _save_note_directly(chat_model, ("python-tips",), "Tips for Python development")

    search_mock = AsyncMock(return_value=[note_match])
    with patch.object(NoteModel, "search_chat_notes", search_mock):
        requests = await test_client.send_command(
            command="search",
            from_user=user_wrapper.user,
            chat=group_chat,
    ...
    assert requests, "Bot should respond to /search command"
    response_text = requests[-1].text or ""
    assert "python-tips" in response_text, f"Response should list matching note, got: {response_text}"
...
async def test_search_notes_no_results(
    ...
    """Searching notes with no matches returns appropriate message."""
    monkeypatch.setattr("sophie_bot.modules.notes.utils.send.bot", test_client.bot)

    user_wrapper, group_chat, _chat_model = await _setup_group_and_user(
        test_client,
        chat_id=-1002800000012,
        user_id=928000012,
        group_title="Notes NoSearch Group",
        first_name="NoSearcher",
        username="no_searcher",
    )
    ...
    with patch.object(NoteModel, "search_chat_notes", search_mock):
        requests = await test_client.send_command(
            ...
            from_user=user_wrapper.user,
            chat=group_chat,
            args="nonexistentquery",
        )

    assert requests, "Bot should respond to /search with no results"
    response_text = requests[-1].text or ""
    assert "No notes found" in response_text or "nonexistentquery" in response_text, (
        f"Response should indicate no search results, got: {response_text}"
    )
