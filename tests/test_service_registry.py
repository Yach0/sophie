...
class FakeServiceRegistry(ServiceRegistry):
    ...
    def _create_mistral(self) -> str:
        self.created_services.append("mistral")
        return "mistral-instance"
...
def test_service_registry_creates_services_lazily_and_caches_instances() -> None:
    ...
    assert registry.get_mistral() == "mistral-instance"
    assert registry.get_scheduler() == "scheduler-instance"

    assert registry.created_services == ["redis", "mongo", "bot", "dispatcher", "mistral", "scheduler"]
...
async def test_service_registry_core_factories_create_clients_without_connecting(
    ...
    try:
        ...
        mistral_client = registry._create_mistral()
        ...
        assert type(mistral_client).__name__ == "Mistral"
