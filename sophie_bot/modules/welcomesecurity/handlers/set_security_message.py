...
@flags.help(description=l_("Sets welcome security message."))
class SetSecurityMessageHandler(SophieMessageHandler):
    ...
    async def handle(self) -> Any:
        ...
        doc = Doc(
            Template(_("Welcome security message was saved in {chat_title}."), chat_title=Italic(connection.title)),
        )

        await self.event.reply(str(doc))


@flags.help(description=l_("Deletes the welcome security message"))
class DelSecurityMessageHandler(SophieMessageHandler):
    ...
    async def handle(self) -> Any:
        ...
        if not db_model or not db_model.security_note:
            return await self.event.reply(
                Template(
                    _("Welcome security message in {chat_title} has not been set before"),
        ...
        doc = Doc(
            Template(
                _("Welcome security message was reset to default in {chat_title}."),
                chat_title=Italic(connection.title),
            ),
        )
