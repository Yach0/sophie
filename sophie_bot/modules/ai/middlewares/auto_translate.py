...
from sophie_bot.shared.restrictive_filter_state import was_restrictive_triggered
...
class AiAutoTranslateMiddleware(BaseMiddleware):
    async def __call__(
        ...
        # Skip autotranslation if a restrictive filter action was triggered
        # on this message (e.g. mute, ban, kick, delete message).
        if isinstance(event, Message) and was_restrictive_triggered(event.message_id):
            log.debug("AiAutoTranslateMiddleware: restrictive filter triggered, skipping autotranslation")
