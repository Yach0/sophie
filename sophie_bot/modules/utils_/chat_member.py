...
async def get_chat_members(chat_tid: int, *, bot: Bot) -> list[ResultChatMemberUnion]:
    return await bot.get_chat_administrators(chat_tid)


async def update_chat_members(chat: ChatModel, *, bot: Bot) -> None:
    chat_members = await get_chat_members(chat.tid, bot=bot)
