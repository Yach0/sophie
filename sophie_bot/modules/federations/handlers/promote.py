...
class FederationPromoteHandler(FederationPromoteDemoteHandler):
    ...
    owner_only_text: ClassVar[str] = l_("Only the federation owner can promote users.")
    user_not_specified_text: ClassVar[str] = l_("Please specify a user to promote or reply to their message.")
    not_private_user_text: ClassVar[str] = l_("Can only promote individual users to admin.")
    success_template: ClassVar[str] = l_("{user} has been promoted to admin of federation {fed_name}.")
    log_template: ClassVar[str] = l_("👤 {admin} promoted {user} to admin in federation {fed_name}.")
